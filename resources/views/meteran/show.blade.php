        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        @can('meteran view')
                        <a href="{{ route('meteran.create') }}"
                            class="btn btn-sm btn-info">
                            <i class="bx bx-plus me-1"></i>Baru
                        </a>
                        @endcan
                        @can('meteran edit')
                        <a href="{{ route('meteran.edit', $meteran) }}"
                            class="btn btn-sm btn-primary">
                            <i class="bx bx-pencil me-1"></i>Edit
                        </a>
                        @endcan
                        @can('meteran delete')
                            <form action="{{ route('meteran.destroy', $meteran) }}"
                                method="POST" class="d-inline">
                                @csrf @method('DELETE')
                                <x-input.confirm-button text="Data meteran ini akan dihapus!"
            ...
            <div class="card-body">
                <form class="row g-3">
                                ...
                                <div class="col-md-8 form-group">: {{ $meteran->id_layanan }}</div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-8 form-group">: {{ $meteran->status }}</div>
                                <div class="col-md-4">
                                    <label for="first-name-horizontal">Chip Kartu</label>
                                </div>
                                <div class="col-md-8 form-group">: {{ $meteran->chip_kartu }}</div>
                </form>
            </div>
        </div>
    </div>
